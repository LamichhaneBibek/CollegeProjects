...
namespace example
{
    public partial class BookTicket : System.Web.UI.Page
    {
        ...
        protected void departon_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DepartureTime.Items.Clear();
                string strsql = "Select * from Time where StationName = '" + Destination.SelectedItem.Text.ToString() + "'";
        ...
        protected void DepartureTime_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (DepartureTime.SelectedItem.Text.ToString() != "--Select--")
            {
                ...
                // display Rent 
                strsql = "Select * from Time where StationName = '" + Destination.SelectedItem.Text.ToString() + "'";
                con = new SqlConnection(DB.ConnectionString);
                cmd = new SqlCommand();
            ...
            rea.Close();
            con.Close();
            strsql = "Select * from Sits where ((Date = '" + departon.SelectedItem.Text.ToString() + "') and (Time ='" + DepartureTime.SelectedItem.Text.ToString() + "')and (Station='" + Destination.SelectedItem.Text.ToString() + "'))";
            con = new SqlConnection(DB.ConnectionString);
            cmd = new SqlCommand();
            con.Open();
            cmd.Connection = con;
            ...
            while (rea.Read())
            {
                for (int j = 1; j <= 31; j++)
                {
                    ...
                    if (rea[s.ToString()].ToString() == "b")
                    {
                        ...
                        img.ImageUrl = "~/images/booked.png";
                        img.Enabled = false;
                    }
        ...
        {
            ibnext.Visible = true;
            if (btn.ImageUrl == "~/images/booked.png")
            {
                seattable.Visible = true;
                btn.ImageUrl = "~/images/waiting.png";
            ...
            else if (btn.ImageUrl == "~/images/waiting.png")
            {
                seattable.Visible = true;
                btn.ImageUrl = "~/images/booked.png";
        ...
        protected void btnSave_Click(object sender, ImageClickEventArgs e)
        {
            ...
            conbal = new SqlConnection(DB.ConnectionString);
            ...
            if (reabal.Read())
            {
                ...
                if (int.Parse(b) >= int.Parse(littotalrent.Text.ToString()))
                {
                    ...
                    string strsql = "select Count(*) from Sits where((Date='" + departon.SelectedItem.Text
                    ...
                    {
                        string sqladd = "INSERT into Sits ([Date],[Time],Station) values('" + departon.SelectedItem.Text
                    ...
                    for (int j = 0; j <= forsave.Length - 1; j++)
                    {
                        ...
                        {
                            string strsql1 = "Update Sits set " + forsave[j].ToString() + " = 'b'  where ((Date='" + departon.SelectedItem.Text.ToString()
                    ...
                    string strsql2 = "Insert Into Passenger values('" + litpnr.Text.ToString() + "','" + txtname.Text.ToString() + "'," + txtphone.Text.ToString() + ",'" + Destination.SelectedItem.Text.ToString() + "','" + Source.SelectedItem.Text.ToString() + "','" + departon.SelectedItem.Text.ToString() + "','" + DepartureTime.SelectedItem.Text.ToString() + "','" + littotalseat.Text.ToString() + "','" + txtseatnumber.Text.ToString() + "','" + littotalrent.Text.ToString() + "','" + User.Identity.Name.ToString() + "','Booked')";
